# build protobuf library for c++, and optionally configure system
# so cmake find_package(Protobuf) will resolve to the built dir

param(
    [Parameter(Mandatory=$false)] [switch] $Clean=$false,
    [Parameter(Mandatory=$false)] [switch] $Install=$true
)
